...
Server::Server(const ServerConfig& cfg) 
    : config(cfg), listen_socket(-1), is_initialized(false),
      static_handler(NULL), cgi_handler(NULL), upload_handler(NULL) {
}
...
std::string Server::getServerInfo() const {
    return "Server " + config.getServerName() + " listening on " + getListenAddress();
}
...
bool Server::needsWriteEvent(int client_fd) const {
    ClientConnection* conn = getConnection(client_fd);
...
bool Server::needsReadEvent(int client_fd) const {
    ClientConnection* conn = getConnection(client_fd);
    if (!conn) return false;
